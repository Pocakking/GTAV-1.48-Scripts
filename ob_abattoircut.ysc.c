...
void __EntryFunction__()//Position - 0x0
{
	...
	while (true)
	{
		...
		if (ENTITY::DOES_ENTITY_EXIST(iScriptParam_0))
		{
			if (TASK::IS_OBJECT_WITHIN_BRAIN_ACTIVATION_RANGE(iScriptParam_0) && SCRIPT::_GET_NUMBER_OF_INSTANCES_OF_SCRIPT_WITH_NAME_HASH(joaat("michael2")) > 0)
			{
				switch (iLocal_2)
				{
					case 0:
						if (ENTITY::DOES_ENTITY_HAVE_DRAWABLE(iScriptParam_0))
						{
							...
							{
								ENTITY::PLAY_ENTITY_ANIM(iScriptParam_0, "beefsplitter_loop", "MISSMIC2", 1f, 1, 0, 0, 0f, 0);
								iLocal_2 = 1;
							}
						}
						break;
